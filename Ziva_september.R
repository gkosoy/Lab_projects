...
backfit <- function(df_train, df_test, conc){
  train_model <- drm(mean_thickness ~ dose, data = df_train, fct = LL.5())
  coef <- train_model$coefficients
  x_test <- coef[4]*(((((coef[1]-coef[3])/(df_test-coef[3]))^(1/coef[5]))-1)^(1/coef[2]))
  difference <- x_test/concentration_2
  return(difference)
  return(x_test)
}

backfit(IL17A_train, IL17A_test_5$mean_thickness, concentration_2)
...
train_model <- drm(mean_thickness ~ dose, data = IL17A_train, fct = LL.5())
coef <- train_model$coefficients
x_test <- coef[4]*(((((coef[1]-coef[3])/(IL17A_test_5$mean_thickness-coef[3]))^(1/coef[5]))-1)^(1/coef[2]))
x_test


#fits standard curve, fits test strip, takes difference
backfit_Wrong <- function(df_train, df_test, x){
  train_model <- drm(mean_thickness ~ dose, data = df_train, fct = LL.5())
  coef <- train_model$coefficients
